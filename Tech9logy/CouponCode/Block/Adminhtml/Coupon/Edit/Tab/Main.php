 ...
 */
namespace Tech9logy\CouponCode\Block\Adminhtml\Coupon\Edit\Tab;

use Tech9logy\CouponCode\Model\Rule;
use Magento\Customer\Api\GroupRepositoryInterface;
use Magento\Framework\Api\SearchCriteriaBuilder;
use Magento\Framework\Convert\DataObject as ObjectConverter;
use Tech9logy\CouponCode\Model\RuleFactory;
    ...
    \Magento\Backend\Block\Widget\Tab\TabInterface
{
    ...
    {
        $model = $this->_coreRegistry->registry("tech9logycouponcode_coupon");
        if ($this->_isAllowedAction("Tech9logy_CouponCode::coupon_edit")) {
            $isElementDisabled = false;
        } else {
            $isElementDisabled = true;
        }

        $this->_eventManager->dispatch("tech9logy_check_license", [
            "obj" => $this,
            "ex" => "Tech9logy_CouponCode",
        ]);

        if ($this->hasData("is_valid") &&
            $this->hasData("local_valid") &&
            !$this->getData("is_valid") &&
            !$this->getData("local_valid")
        ) {
            $isElementDisabled = true;
        ...
        if ($model->getId()) {
            $fieldset->addField("couponcode_id", "hidden", [
                "name" => "couponcode_id",
            ]);
        }
        ...
        $fieldset->addField("time_used", "text", [
            "name" => "time_used",
            "label" => __("Time Used"),
            "title" => __("Time Used"),
            "required" => false,
        ...
        $fieldset->addField("expiration_date", "text", [
        ...
        $couponRule = $modelRule
            ->addFieldToFilter("rule_id", $model->getRuleId())
            ->getFirstItem()
            ->getData();
        ...
        if ($model->getCustomerId()) {
            $customer_text =
                '<a href="' .
                $this->getUrl("customer/index/edit", [
