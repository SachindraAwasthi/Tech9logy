...
class AbstractGrid extends \Magento\Backend\Block\Widget\Grid\Extended
{
    ...
    {
        if (is_array($column) &&
            array_key_exists("visibility_filter", $column)
        ) {
    ...
    protected function _getStoreIds()
    {
        ...
        if ($filterData) {
            ...
            $storeIds = !empty($storeIdsString)
                ? explode(",", $storeIdsString)
                : [];
        } else {
            $storeIds = [];
        }
    ...
    public function getCurrentCurrencyCode()
    {
        if ($this->_currentCurrencyCode === null) {
            $this->_currentCurrencyCode =
                count($this->_storeIds) > 0
                    ? $this->_storeManager
                        ->getStore(array_shift($this->_storeIds))
                        ->getBaseCurrencyCode()
    ...
    {
        $collection->addOrderStatusFilter(
            $filterData->getData("order_statuses")
        );
        return $this;
    }
        ...
        if ($custom_filters) {
            foreach ($custom_filters as $name => $value) {
                if ($custom_filters[$name] &&
                    is_string($custom_filters[$name])
                ) {
                    $collection->addFieldToFilter($name, [
                        "like" => "%" . $custom_filters[$name] . "%",
                    ]);
                } elseif ($custom_filters[$name] &&
                    is_array($custom_filters[$name])
                ) {
                    $from_value = isset($custom_filters[$name][0])
                        ? $custom_filters[$name][0]
                        : false;
                    $from_value =
                        $from_value == false &&
                        isset($custom_filters[$name]["from"])
                            ? $custom_filters[$name]["from"]
                            : "";
                    $to_value = isset($custom_filters[$name][1])
                        ? $custom_filters[$name][1]
                        : false;
                    $to_value =
                        $to_value == false &&
                        isset($custom_filters[$name]["to"])
                            ? $custom_filters[$name]["to"]
                            : "";
