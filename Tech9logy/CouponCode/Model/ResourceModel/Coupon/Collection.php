 ...
 */
namespace Tech9logy\Couponcode\Model\ResourceModel\Coupon;
...
class Collection extends AbstractCollection
{
    ...
    {
        $this->_init('\Tech9logy\CouponCode\Model\Coupon', '\Tech9logy\CouponCode\Model\ResourceModel\Coupon');
    }
        ...
        if (count($items)) {
            ...
            foreach ($this as $item) {
                ...
                $select = $connection->select()
                                    ...
                                    ->join(
                                        ['tech9logyrule' => $this->getTable('tech9logy_couponcode_rule')],
                                        'salesrule.rule_id = tech9logyrule.rule_id',
                                        "coupon_rule_id"
                                    )
                                    ->where(
                                        'salesrule.rule_id = (?)',
    ...
    public function getByCouponCode($coupon_code)
    {
        ...
        $SellerPost = $objectManager->get('\Tech9logy\CouponCode\Model\Coupon')->load($coupon_code, 'code');
    ...
    public function getCouponCodeByConditions($param)
    {
        ...
        $select = 'SELECT *  FROM ' . $this->getTable('salesrule_coupon') . ' INNER JOIN '. $this->getTable('tech9logy_couponcode_coupon').' ON salesrule_coupon.coupon_id = tech9logy_couponcode_coupon.couponcode_id';
        $where = '';
        $numItems = count($param);
        $i = 0;
        foreach ($param as $key => $value) {
            if (++$i === $numItems) {
                $where .=' tech9logy_couponcode_coupon.'. $key . ' = ' . '"' . $value . '"';
            } else {
                $where .=' tech9logy_couponcode_coupon.'. $key . ' = ' . '"' . $value . '" and ';
            }
        }
        if ($where != '') {
            $where = ' WHERE '. $where;
